...
def train(attn_implementation=None):
    ...
    seg_ckpt_dir = 'results/pretrain/llama-seg_query_generator_-11_-2'
    audio_ckpt_dir = 'results/pretrain/llama-audio-qformer'
    ...
    # ckpt = torch.load(join(seg_ckpt_dir,'non_lora_trainables.bin'),map_location='cpu')
    # # ckpt = torch.load(join(seg_ckpt_dir,'seg_module_pretrain_weights.bin'),map_location='cpu')
    # embed_tokens = ckpt.pop('model.embed_tokens.weight',None)
    # lm_head = ckpt.pop('lm_head.weight',None)
    # if embed_tokens is not None:
    #     print(f'pop embed_tokens weight, shape: {embed_tokens.shape}')
    # if lm_head is not None:
    #     print(f'pop lm_head weights, shape: {lm_head.shape}')
    # if data_args.avss_task:
    #     avs_classifier_name = 'model.seg_module.mask_decoder.avs_classfier.weight'
    #     weight = ckpt.pop(avs_classifier_name,None)
    #     if weight is not None:
    #         print('avss task, pop avs_classifier_name weight, shape: ',weight.shape)
    # model.model.load_state_dict(ckpt,strict=False)
