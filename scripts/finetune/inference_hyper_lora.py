...
def inference_avqa(dataloader,ckpt_dir,model,tokenizer):
    ...
    for step, sample in enumerate(dataloader):
        ...
        for i in range(bs):
            ...
            # metadata['q_token_avg'] = model.model.q_token_avg.tolist()
            # metadata['k_token_avg'] = model.model.k_token_avg.tolist()
            # metadata['v_token_avg'] = model.model.v_token_avg.tolist()
            # metadata['o_token_avg'] = model.model.o_token_avg.tolist()
            # metadata['token_nums'] = model.model.token_nums

            write2json(fp=fp,dict_data=metadata)

            # model.model.q_token_avg = 0
            # model.model.k_token_avg = 0
            # model.model.v_token_avg = 0
            # model.model.o_token_avg = 0
...
def inference_ms3(dataloader,ckpt_dir,model,tokenizer):
    ...
    f1 = 0.
    count = 0
    fp = join(save_dir,'inference_results_2.jsonl')
    for step,sample in enumerate(dataloader):
        batch_metadata = sample.pop('batch_metadata')
        image_path = batch_metadata[0]['image_path']
        mask_path = batch_metadata[0]['mask_path']
...
def train(attn_implementation=None):
    ...
    if not infer_avs:
        ...
        adapter_weights = infer_args.adapter_ckpt_path
        ckpt = torch.load(adapter_weights,map_location='cpu')
        model.load_state_dict(ckpt,strict=False)
        print(f'load hyper_lora weights from {adapter_weights} finished...')

        ckpt_path = join(ckpt_dir,'seg_module_weights.bin')
        ckpt = torch.load(ckpt_path,map_location='cpu')
        model.load_state_dict(ckpt,strict=False)
    ...
    if data_args.s4_task:
        # inference_s4_vqgan(dataloader=dataloader,ckpt_dir=ckpt_dir,model=model,tokenizer=tokenizer)
    ...
    if data_args.avss_task:
        # inference_avs(dataloader=dataloader,ckpt_dir=ckpt_dir,model=model,tokenizer=tokenizer)
